...
public class CustomAdvancedImportServiceImpl implements CustomAdvancedImportService {
  ...
  @Override
  public void removeJsonRecords(List<FileTab> fileTabList)
    ...
    for (FileTab fileTab : fileTabList) {
      ...
      String fieldName = "JSON_" + fileTab.getJsonModel().getName();
      ...
      removeJsonRecord(fileTab, modelKlass, ids, fileTabList);
      removeJsonSubRecords(modelKlass, fileTab.getJsonModel().getFields(), jsonObject);
    }
  }
  ...
  public void removeJsonRecord(
    ...
    for (FileTab tab : fileTabList) {
      ...
      String fieldName = "JSON_" + jsonModel.getName();
      ...
      for (MetaJsonField jsonField : tab.getJsonModel().getFields()) {
        if (jsonField.getTargetJsonModel() != null
          ...
          removeJsonRecord(tab, modelKlass, ids, fileTabList);
        }
      }
    }
  ...
  public void removeJsonSubRecords(
      Class<? extends Model> klass, List<MetaJsonField> jsonFields, JSONObject jsonObject)
      throws ClassNotFoundException, JSONException {
    ...
    for (MetaJsonField jsonField : jsonFields) {
      ...
      String simpleModelName =
          Strings.isNullOrEmpty(jsonField.getTargetModel())
              ? "JSON_" + jsonField.getTargetJsonModel().getName()
